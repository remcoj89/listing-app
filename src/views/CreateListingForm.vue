      ...
      <form @submit.prevent="submit">
        ...
        <div class="input-field">
          ...
          <label class="upload-box" for="upload">
            <img v-if="selectedFile" :src="imageUrl" class="uploading-image" alt="uploaded image" />
            <img id="upload-image" v-else src="@/assets/icons/ic_plus_grey@3x.png" alt="" />
            <input
              name="image"
              id="upload"
              type="file"
...
export default {
  ...
  methods: {
    submit() {
      ...
      createListing(formdata).then((responseData) => {
        this.uploadImage(responseData.id)
      })
    },
    ...
    uploadImage(id) {
      ...
      uploadListingImage(id, formdata)
    ...
    validateInput(input) {
      if (input === 'streetName') {
        ...
        this.houseNumberValidity = this.houseNumber === null ? 'invalid' : 'valid'
      } else if (input === 'addition') {
        this.additionValidity = this.addtion === '' ? 'invalid' : 'valid'
      } else if (input === 'zip') {
        this.zipValidity = this.zip === '' ? 'invalid' : 'valid'
      } else if (input === 'city') {
        this.cityValidity = this.city === '' ? 'invalid' : 'valid'
      } else if (input === 'price') {
        this.priceValidity = this.price === null ? 'invalid' : 'valid'
      } else if (input === 'size') {
        this.sizeValidity = this.size === null ? 'invalid' : 'valid'
      } else if (input === 'garage') {
        this.garageValidity = this.garage === null ? 'invalid' : 'valid'
      } else if (input === 'bedroom') {
        this.bedroomValidity = this.bedroom === null ? 'invalid' : 'valid'
      } else if (input === 'bathroom') {
        this.bathroomValidity = this.bathroom === null ? 'invalid' : 'valid'
      } else if (input === 'constructionyear') {
        this.constructionYearValidity = this.constructionYear === null ? 'invalid' : 'valid'
      } else if (input === 'description') {
        this.descriptionValidity = this.description === '' ? 'invalid' : 'valid'
      }
    },
